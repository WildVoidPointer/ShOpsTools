...
readonly SYSV_INIT_DEFAULT_SCRIPT='./mysql.server'
...
install_mysql_binary() {
    ...
    is_exist "$SYSV_INIT_DEFAULT_SCRIPT"

    mkdir -p "$MYSQL_INSTALL_PATH"
    
    tar -zxvf "$MYSQL_PKG_PATH" -C "$MYSQL_INSTALL_PATH" --strip-components=1 &>/dev/null \
    ...
    chmod 600 "${MYSQL_INSTALL_PATH}/conf/my.cnf"
...
EOF
    ...
    cp "$SYSV_INIT_DEFAULT_SCRIPT" "${MYSQL_INSTALL_PATH}/mysql.server"
    chown root:root "${MYSQL_INSTALL_PATH}/mysql.server"
    chmod 700 "${MYSQL_INSTALL_PATH}/mysql.server"
...
####################################
# Create symbolic links required for FHS compliance and SysV init script.
#
# Globals:
#   MYSQL_INSTALL_PATH
#   MYSQL_FHS_LINK_PATH
...
create_symlinks() {
    ...
    # 1. Create /usr/local/mysql -> /www/services/mysql57
    ...
    # 2. Create /etc/init.d/mysql -> mysql.server
    local init_script="${SYSV_INIT_SCRIPTS_PATH}/mysql"

    if [[ -L "$init_script" ]] || [[ -e "$init_script" ]]; then
        rm -rf "$init_script" || log_error "Failed to remove existing $init_script"
    fi
    ln -s "${MYSQL_INSTALL_PATH}/mysql.server" "$init_script" \
        || log_error "Failed to create symlink $init_script"

    log_info "Symlink $init_script -> $MYSQL_INSTALL_PATH/mysql.server created"
}
...
initialize_and_start_mysql() {
    ...
    log_info "Starting MySQL service via 'service mysql start'..."
    service mysql start || log_error "Failed to start MySQL service"
    sleep 2

    # Check service status
    log_info "Checking MySQL service status..."
    service mysql status || log_error "MySQL service status check failed"
